...
const ProductCard = ({ product }) => {
  const { currency, addToCart, removeFromCart, navigate, cartItems, updateCartItem } =
    useAppContext();
  ...
  const routePrice = product.price ?? product.offerPrice ?? 0;
  ...
  return (
    ...
    >
      <div
        onClick={() => {
          navigate(`/products/${category}/${routePrice}/${productId}`);
          scrollTo(0, 0);
        }}
      ...
      >
        <div className="group flex items-center justify-center px-2">
          <img
            ...
            src={product.images?.[0] || product.image?.[0] || assets.placeholder_image}
            alt={product.name}
          />
        </div>

        <div className="text-gray-500/60 text-sm mt-2">
          <p className="capitalize">{String(category).replace(/-/g, " ")}</p>
            ...
            <div onClick={(e) => e.stopPropagation()} className="text-primary">
              {!qty ? (
                ...
                <div className="flex items-center justify-center gap-2 md:w-20 w-16 h-[34px] bg-primary/25 rounded select-none">
                  ...
                  <button
                    onClick={() =>
                      updateCartItem(productId, cartItems[productId].quantity + 1)
                    }
                    className="cursor-pointer text-md px-2 h-full"
                  >
                    +
